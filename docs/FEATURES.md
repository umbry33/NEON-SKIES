...
| 100 关卡模式 | 100 个可选战区，每 5 关 Boss，26 关后环境，51 关后原型敌池 | `src/config/level-config.js`、`src/config/enemy-config.js`、`src/core/Game.js` | 已完成；测试页仍有旧的 50 关断言 |
...
| 光锥之外航程 | 随机路线、四档难度、多章节、节点、事件、商店、回复、库存、奖励和 Boss | `src/systems/BeyondLightConeSystem.js`、`src/ui/BeyondLightConeUI.js`、`src/config/beyond-*` | 已完成；UI 文案仍把 34 个事件写成 30 个，见已知问题 |
