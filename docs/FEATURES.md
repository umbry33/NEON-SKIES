...
| 浏览器本地存档与普通机体机库 | 当前普通机体自动保存到浏览器本地，并提供 10 个机库槽位切换；点击名称可重命名；每个槽位可直接导入/导出配置码；光锥之外不使用机库 | `src/systems/LocalSaveSystem.js`、`src/ui/UI.js`、`src/systems/LoadoutCodec.js` | 已完成；名称最多 24 个字符，仅限当前浏览器设备，跨设备仍使用配置码 |
...
| 光锥之外航程 | 随机路线、四档难度、多章节、节点、事件、商店、回复、库存、奖励和 Boss | `src/systems/BeyondLightConeSystem.js`、`src/ui/BeyondLightConeUI.js`、`src/config/beyond-*` | 已完成；当前事件档案为 39 个；5 个仍为传说稀有度且未进入普通模块池的模块仅通过低概率稀有事件定向获取，开放的稀有/史诗模块进入标准随机模块池 |
...
| 合成数据与系统 | 暂停开发的旧配方、库存合成/分解代码 | `src/config/module-config.js`、`BeyondLightConeSystem.js`、`BeyondLightConeUI.js` | 暂停开发；不对玩家开放，当前没有“合成模块”定义，也不进入玩家功能清单 |
| Canvas 图形与音频 | 几何图标、特效、背景、项目根目录 MP3 背景音乐、Web Audio 音效和音量控制 | `src/rendering/ModuleRenderer.js`、各实体/场景 renderer、`SoundSystem.js` | 已完成；背景音乐资源由发布脚本复制 |
| 游戏内百科与更新日志 | 查看模块、敌机、Boss、环境、光锥之外规则和历史更新记录 | `src/config/encyclopedia-config.js`、`src/ui/UI.js`、`index.html` | 已完成；部分历史文案需持续核对 |

## 标准装配内容

当前改装仓库由配置渲染为 26 个玩家可选模块，其中稀有/史诗标准投放内容包括“星弧”“光学迷踪”“幽蓝奇点”“静默磁暴”和“战术地雷”；战术地雷使用 3 层充能，6 秒恢复 1 层，每枚最多存在 30 秒。暂停开发的旧合成定义不计入玩家模块。完整模块/字段清单见 `docs/DATA_MODEL.md`。新增模块必须同时更新百科、图标、行为和测试，不要只更新 README。

## 未实现或不应误写成已完成

故事、联网、账号、排行榜、数据库、云端同步、模块掉落、战斗中替换、模块旋转和完整浏览器兼容矩阵均未被当前代码证明为已完成；本项目当前不计划维护完整矩阵。
