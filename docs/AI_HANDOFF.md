...
- 最近代码提交：`b3fe96b`，`fix: 修复机库按钮移动端触控并补充发布检查`；后续提交以 Git 历史为准。
- 当前阶段：v0.8.0 内容扩展完成后的持续开发/稳定化阶段；当前修复聚焦移动端机库触控和发布前触控回归要求。
...
- 已修复改装界面工具按钮在移动端触控目标过小的问题，并为仅提供 Touch Events 的旧移动浏览器补充按钮释放处理；普通模式机库按钮仍只在普通改装界面显示。
...
- 本地浏览器回归已确认：改装界面机库按钮可点按打开，弹窗显示 10 个槽位；测试页显示 `All tests passed`。下一次正式发布仍须按清单在手机或移动端模拟环境完成触控回归并记录结果。
