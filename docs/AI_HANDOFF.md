...
- 最近代码提交：本次 v0.8.1 发布提交以 Git 历史为准。
- 当前阶段：v0.8.1 已完成本地发布准备；本版本聚焦移动端机库触控和发布前触控回归要求。
...
- 逻辑测试页最近一次运行结果为 `All tests passed`，共 75 项测试。
- `src/ui/UI.js`、`src/main.js` 通过 `node --check`，工作区差异通过 `git diff --check`。
- 发布目录共 38 个源/配置/资源文件与工作区对应文件哈希一致；本地测试地址返回 HTTP 200。
- 本地浏览器回归已确认：390×844 手机尺寸下工具按钮恢复紧凑样式，机库按钮可点按打开，弹窗显示 10 个槽位；测试页显示 `All tests passed`。下一次正式发布仍须按清单在手机或移动端模拟环境完成触控回归并记录结果。
...
2. 每次正式发布前按 `docs/TEST_CHECKLIST.md` 做一台电脑和一台手机（或移动端模拟环境）的轻量冒烟测试，必须包含机库按钮和其它触控路径；不要自行扩展成复杂兼容矩阵。
