...
export class Wingman {
  constructor(player, { wolfpack = false } = {}) {
    this.x = player.x - 42;
    this.y = player.y + 4;
    this.hp = 10;
    this.maxHp = 10;
  ...
  canFireNest() { return this.active && this.wolfpack && this.nestTimer <= 0; }
  armNest(interval) { this.nestTimer = interval; }

  damage(amount) { if (!this.active) return; this.hp = Math.max(0, this.hp - amount); if (this.hp === 0) this.respawnTimer = 10; }

  draw(ctx) {
    if (!this.active) return;
    ctx.save(); ctx.translate(this.x, this.y); ctx.globalAlpha = 0.9;
    const color = this.wolfpack ? "#ef294d" : "#b9f8ff";
    ctx.shadowBlur = 14; ctx.shadowColor = color; ctx.fillStyle = this.wolfpack ? "#18090f" : color;
    ctx.beginPath(); ctx.moveTo(0, -13); ctx.lineTo(11, 9); ctx.lineTo(0, 5); ctx.lineTo(-11, 9); ctx.closePath(); ctx.fill();
    if (this.wolfpack) { ctx.lineWidth = 2; ctx.strokeStyle = "#ff5870"; ctx.stroke(); ctx.fillStyle = "#ef294d"; ctx.fillRect(-11, 6, 5, 3); ctx.fillRect(6, 6, 5, 3); ctx.fillRect(-3, -3, 6, 7); }
    ctx.restore();
  }
}
