...
export class UI {
  constructor() {
    ...
    this.loadout = createLoadout(); this.history = []; this.dragState = null; this.suppressClick = false; this.instanceCounter = 1; this.selectedMode = "endless"; this.selectedLevel = 1; this.quickAssemblyEnabled = false; this.quickAssemblyModuleId = null; this.tutorialActionsDone = new Set(); this.tutorialContext = null; this.tutorialBuilderInitialSignature = "";
    this.boardView = { scale: 1, offsetX: 0, offsetY: 0 }; this.boardPointers = new Map(); this.boardGesture = null;
    ...
    bindTap(this.moreMenuButton, () => this.toggleMoreMenu()); document.addEventListener("pointerup", (event) => this.dismissMoreMenuOutside(event), true); document.addEventListener("click", (event) => this.dismissMoreMenuOutside(event), true); bindTap(this.settingsButton, () => { this.hideMoreMenu(); this.settingsModal.classList.remove("is-hidden"); }); bindTap(this.settingsClose, () => this.settingsModal.classList.add("is-hidden")); bindTap(this.changelogButton, () => { this.hideMoreMenu(); void this.showChangelog(); }); bindTap(this.changelogClose, () => this.hideChangelog()); this.settingsModal.addEventListener("click", (event) => { if (event.target === this.settingsModal) this.settingsModal.classList.add("is-hidden"); }); this.changelogModal.addEventListener("click", (event) => { if (event.target === this.changelogModal) this.hideChangelog(); }); bindSettingToggle(this.damageNumberToggle, (enabled) => this.onDamageNumbersChanged?.(enabled)); bindSettingToggle(this.vibrationToggle, (enabled) => this.onVibrationChanged?.(enabled)); bindSettingToggle(this.soundToggle, (enabled) => this.onSoundChanged?.(enabled)); bindSettingToggle(this.moduleGlowToggle, (enabled) => { setModuleGlowEnabled(enabled); this.paintIcons(this.moduleLibrary); this.paintIcons(this.assemblyBoard); });
    ...
    this.assemblyBoard.addEventListener("pointermove", (event) => { if (!this.moveBoardGesture(event)) this.updateQuickAssemblyPreview(event); }, { passive: false });
    ...
    this.assemblyBoard.addEventListener("pointerup", (event) => {
      ...
      if (!isTouchLikePointer(event) || !this.quickAssemblyEnabled || !tap) return;
      event.preventDefault();
      event.stopPropagation();
  ...
  bind({ onStart, onMenu, onPause, onResume, onSkill, onDamageNumbersChanged, onVibrationChanged, onSoundChanged, onNextLevel, onLevelSelect, onModeSelect, onTutorialBattleExit, onEnvironmentPause, onEnvironmentResume }) {
  ...
  importLoadoutCode() { try { const { loadout, moduleCount } = decodeLoadoutCode(this.loadoutCodeInput.value); this.pushHistory(); this.loadout = loadout; this.instanceCounter = Math.max(this.instanceCounter, moduleCount + 1); this.renderAssembly(); this.hideLoadoutCode(); this.setStatus(`已导入 ${moduleCount} 个模块`); } catch (error) { this.loadoutCodeMessage.textContent = error instanceof Error ? error.message : "配置码无效"; } }
   getSelectedIds() { return { modules: this.loadout.modules.map(({ instanceId, moduleId, x, y, rotation }) => ({ instanceId, moduleId, x, y, rotation })), mode: this.selectedMode, level: this.selectedLevel, dodgeDifficulty: this.selectedDodgeDifficulty }; }
  ...
  transitionToScreen(target, { direction = "forward", animate = true } = {}) {
    const screens = [this.menuScreen, this.modeScreen, this.dodgeDifficultyScreen, this.tutorialScreen, this.levelSelectScreen, this.encyclopediaScreen, this.builderScreen, this.battleScreen].filter(Boolean);
  ...
  hideAllScreens() {
    ...
    if (this.screenTransitionTimer) window.clearTimeout(this.screenTransitionTimer);
    this.screenTransitionTimer = null;
    [this.menuScreen, this.modeScreen, this.dodgeDifficultyScreen, this.tutorialScreen, this.levelSelectScreen, this.encyclopediaScreen, this.builderScreen, this.battleScreen].forEach((screen) => {
  ...
  setBoardView(scale, offsetX = this.boardView.offsetX, offsetY = this.boardView.offsetY) {
    const nextScale = Math.max(.65, Math.min(2.8, scale)); const clamped = this.clampBoardOffset(offsetX, offsetY, nextScale); this.boardView = { scale: nextScale, ...clamped }; this.applyBoardView();
  }
  zoomBoardAt(factor, clientX = null, clientY = null) {
    const rect = this.assemblyBoard.getBoundingClientRect(); const centerX = clientX == null ? rect.width / 2 : clientX - rect.left; const centerY = clientY == null ? rect.height / 2 : clientY - rect.top; const nextScale = Math.max(.65, Math.min(2.8, this.boardView.scale * factor)); const worldX = (centerX - this.boardView.offsetX) / this.boardView.scale; const worldY = (centerY - this.boardView.offsetY) / this.boardView.scale; this.setBoardView(nextScale, centerX - worldX * nextScale, centerY - worldY * nextScale);
  }
