...
export class UI {
  constructor() {
    ...
    bindTap(this.moreMenuButton, () => this.toggleMoreMenu()); document.addEventListener("pointerup", (event) => this.dismissMoreMenuOutside(event), true); document.addEventListener("click", (event) => this.dismissMoreMenuOutside(event), true); bindTap(this.settingsButton, () => { this.hideMoreMenu(); this.settingsModal.classList.remove("is-hidden"); }); bindTap(this.settingsClose, () => this.settingsModal.classList.add("is-hidden")); bindTap(this.changelogButton, () => { this.hideMoreMenu(); void this.showChangelog(); }); bindTap(this.changelogClose, () => this.hideChangelog()); this.settingsModal.addEventListener("click", (event) => { if (event.target === this.settingsModal) this.settingsModal.classList.add("is-hidden"); }); this.changelogModal.addEventListener("click", (event) => { if (event.target === this.changelogModal) this.hideChangelog(); }); bindSettingToggle(this.damageNumberToggle, (enabled) => this.onDamageNumbersChanged?.(enabled)); bindSettingToggle(this.vibrationToggle, (enabled) => this.onVibrationChanged?.(enabled)); bindVolumeRange(this.musicVolumeInput, this.musicVolumeValue, (value) => this.onMusicVolumeChanged?.(value)); bindVolumeRange(this.soundVolumeInput, this.soundVolumeValue, (value) => this.onSoundVolumeChanged?.(value)); bindSettingToggle(this.moduleGlowToggle, (enabled) => { setModuleGlowEnabled(enabled); this.paintIcons(this.moduleLibrary); this.paintIcons(this.assemblyBoard); });
  ...
  bind({ onStart, onMenu, onPause, onResume, onSkill, onDamageNumbersChanged, onVibrationChanged, onMusicVolumeChanged, onSoundVolumeChanged, onNextLevel, onBeyondStageContinue, onLevelSelect, onModeSelect, onTutorialBattleExit, onEnvironmentPause, onEnvironmentResume }) {
    this.onStart = onStart; this.onMenu = onMenu; this.onPause = onPause; this.onResume = onResume; this.onSkill = onSkill; this.onDamageNumbersChanged = onDamageNumbersChanged; this.onVibrationChanged = onVibrationChanged; this.onMusicVolumeChanged = onMusicVolumeChanged; this.onSoundVolumeChanged = onSoundVolumeChanged; this.onNextLevel = onNextLevel; this.onBeyondStageContinue = onBeyondStageContinue; this.onLevelSelect = onLevelSelect; this.onModeSelect = onModeSelect; this.onTutorialBattleExit = onTutorialBattleExit; this.onEnvironmentPause = onEnvironmentPause; this.onEnvironmentResume = onEnvironmentResume;
