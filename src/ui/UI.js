...
import { getBeyondEventArchiveEntries, getEncyclopediaSections, getUnreleasedEnemyArchiveEntries } from "../config/encyclopedia-config.js";
...
import { HANGAR_SLOT_COUNT, LocalSaveSystem } from "../systems/LocalSaveSystem.js";
...
function paintEncyclopediaEnemy(canvas, enemy) {
  ...
  const ctx = canvas.getContext("2d"); const { type, color, flipY = false } = enemy;
  const size = canvas.width; const center = size / 2;
  ctx.clearRect(0, 0, size, size);
  ctx.save(); ctx.translate(center, center); ctx.scale(size / 62, (flipY ? -1 : 1) * size / 62);
  ctx.shadowBlur = 12; ctx.shadowColor = color; ctx.fillStyle = color;
  ctx.beginPath();
  if (type === "storm") { ctx.moveTo(0, -25); ctx.lineTo(9, -11); ctx.lineTo(23, -13); ctx.lineTo(14, 0); ctx.lineTo(24, 14); ctx.lineTo(8, 11); ctx.lineTo(0, 25); ctx.lineTo(-8, 11); ctx.lineTo(-24, 14); ctx.lineTo(-14, 0); ctx.lineTo(-23, -13); ctx.lineTo(-9, -11); }
  else if (type === "nest") { ctx.moveTo(0, -24); ctx.lineTo(15, -13); ctx.lineTo(27, 4); ctx.lineTo(13, 15); ctx.lineTo(0, 24); ctx.lineTo(-13, 15); ctx.lineTo(-27, 4); ctx.lineTo(-15, -13); }
  else if (type === "prism") { ctx.moveTo(0, -26); ctx.lineTo(23, 0); ctx.lineTo(0, 26); ctx.lineTo(-23, 0); }
  else if (type === "cryo") { ctx.moveTo(0, -26); ctx.lineTo(10, -15); ctx.lineTo(23, -13); ctx.lineTo(16, 0); ctx.lineTo(23, 13); ctx.lineTo(10, 15); ctx.lineTo(0, 26); ctx.lineTo(-10, 15); ctx.lineTo(-23, 13); ctx.lineTo(-16, 0); ctx.lineTo(-23, -13); ctx.lineTo(-10, -15); }
  else if (type === "lattice") { ctx.moveTo(0, -25); ctx.lineTo(19, -17); ctx.lineTo(26, 0); ctx.lineTo(19, 17); ctx.lineTo(0, 25); ctx.lineTo(-19, 17); ctx.lineTo(-26, 0); ctx.lineTo(-19, -17); }
  else if (type === "abyss") { ctx.moveTo(0, -27); ctx.quadraticCurveTo(9, -14, 24, -17); ctx.lineTo(15, 0); ctx.quadraticCurveTo(25, 11, 10, 12); ctx.quadraticCurveTo(0, 27, -10, 12); ctx.quadraticCurveTo(-25, 11, -15, 0); ctx.lineTo(-24, -17); ctx.quadraticCurveTo(-9, -14, 0, -27); }
  else if (type === "chorus") { ctx.arc(0, 0, 22, 0, Math.PI * 2); }
  else if (type === "hive") { ctx.moveTo(0, -26); ctx.lineTo(22, -13); ctx.lineTo(22, 13); ctx.lineTo(0, 26); ctx.lineTo(-22, 13); ctx.lineTo(-22, -13); }
  else if (type === "default") { ctx.arc(0, 0, 25, 0, Math.PI * 2); }
  else if (type === "guardian") { ctx.moveTo(0, -23); ctx.lineTo(20, -8); ctx.lineTo(16, 17); ctx.lineTo(0, 23); ctx.lineTo(-16, 17); ctx.lineTo(-20, -8); }
  ...
  else if (type === "prism") { ctx.moveTo(0, -24); ctx.lineTo(21, 0); ctx.lineTo(0, 24); ctx.lineTo(-21, 0); }
  else if (type === "anchor") { ctx.moveTo(-18, -17); ctx.lineTo(18, -17); ctx.lineTo(12, 9); ctx.lineTo(0, 23); ctx.lineTo(-12, 9); }
  else if (type === "echo") { ctx.moveTo(0, 22); ctx.lineTo(19, -15); ctx.lineTo(4, -9); ctx.lineTo(0, -23); ctx.lineTo(-4, -9); ctx.lineTo(-19, -15); }
  else if (type === "loom") { ctx.moveTo(0, -23); ctx.lineTo(19, -10); ctx.lineTo(19, 12); ctx.lineTo(0, 23); ctx.lineTo(-19, 12); ctx.lineTo(-19, -10); }
  else if (type === "lantern") { ctx.rect(-15, -14, 30, 31); }
  else if (type === "comet") { ctx.moveTo(0, -25); ctx.lineTo(17, -4); ctx.lineTo(7, 21); ctx.lineTo(0, 13); ctx.lineTo(-7, 21); ctx.lineTo(-17, -4); }
  else if (type === "pendulum") { ctx.moveTo(0, -22); ctx.lineTo(15, -5); ctx.lineTo(12, 16); ctx.lineTo(0, 24); ctx.lineTo(-12, 16); ctx.lineTo(-15, -5); }
  else if (type === "ribbon") { ctx.moveTo(0, -22); ctx.quadraticCurveTo(28, -10, 19, 6); ctx.quadraticCurveTo(10, 22, 0, 14); ctx.quadraticCurveTo(-10, 22, -19, 6); ctx.quadraticCurveTo(-28, -10, 0, -22); }
  else if (type === "drum") { ctx.arc(0, 0, 21, 0, Math.PI * 2); }
  else if (type === "petal") { ctx.moveTo(0, -23); ctx.quadraticCurveTo(19, -18, 11, -2); ctx.quadraticCurveTo(24, 7, 7, 11); ctx.quadraticCurveTo(0, 25, -7, 11); ctx.quadraticCurveTo(-24, 7, -11, -2); ctx.quadraticCurveTo(-19, -18, 0, -23); }
  ...
  if (type === "default") { ctx.strokeStyle = "#ffb0c0"; ctx.lineWidth = 2; ctx.shadowColor = color; ctx.shadowBlur = 14; ctx.beginPath(); ctx.arc(0, 0, 29, 0, Math.PI * 2); ctx.stroke(); }
  if (type === "storm") { ctx.strokeStyle = "#ecffff"; ctx.lineWidth = 2; ctx.beginPath(); ctx.moveTo(-16, -14); ctx.lineTo(-5, -3); ctx.lineTo(-13, 7); ctx.moveTo(16, -14); ctx.lineTo(5, -3); ctx.lineTo(13, 7); ctx.stroke(); }
  if (type === "nest") { ctx.strokeStyle = "#ffd1dc"; ctx.lineWidth = 2; ctx.beginPath(); ctx.moveTo(-24, 4); ctx.lineTo(-11, 4); ctx.moveTo(24, 4); ctx.lineTo(11, 4); ctx.moveTo(-16, -12); ctx.lineTo(0, 0); ctx.lineTo(16, -12); ctx.stroke(); }
  if (type === "cryo") { ctx.strokeStyle = "#efffff"; ctx.lineWidth = 1.8; ctx.beginPath(); ctx.moveTo(-16, 0); ctx.lineTo(16, 0); ctx.moveTo(0, -16); ctx.lineTo(0, 16); ctx.moveTo(-11, -11); ctx.lineTo(11, 11); ctx.moveTo(11, -11); ctx.lineTo(-11, 11); ctx.stroke(); }
  if (type === "lattice") { ctx.strokeStyle = "#f3ddff"; ctx.lineWidth = 1.8; ctx.strokeRect(-14, -14, 28, 28); ctx.strokeRect(-7, -7, 14, 14); }
  if (type === "abyss") { ctx.strokeStyle = "#1a082b"; ctx.lineWidth = 2; ctx.beginPath(); ctx.arc(0, 0, 14, 0, Math.PI * 2); ctx.stroke(); ctx.globalAlpha = .55; ctx.strokeStyle = "#f0c9ff"; ctx.beginPath(); ctx.arc(0, 0, 25, .2, 2.8); ctx.stroke(); ctx.globalAlpha = 1; }
  if (type === "chorus") { ctx.strokeStyle = "#fffbe0"; ctx.lineWidth = 2; ctx.beginPath(); ctx.arc(0, 0, 13, 0, Math.PI * 2); ctx.stroke(); ctx.beginPath(); ctx.arc(0, 0, 28, -.8, .8); ctx.arc(0, 0, 28, Math.PI - .8, Math.PI + .8); ctx.stroke(); }
  if (type === "hive") { ctx.strokeStyle = "#effffb"; ctx.lineWidth = 1.8; ctx.beginPath(); ctx.moveTo(-11, -12); ctx.lineTo(0, -18); ctx.lineTo(11, -12); ctx.lineTo(11, 0); ctx.lineTo(0, 6); ctx.lineTo(-11, 0); ctx.closePath(); ctx.stroke(); ctx.beginPath(); ctx.moveTo(-11, 7); ctx.lineTo(0, 13); ctx.lineTo(11, 7); ctx.stroke(); }
...
export class UI {
  constructor() {
    ...
    this.hangarButton = document.querySelector("#hangar-button"); this.hangarModal = document.querySelector("#hangar-modal"); this.hangarClose = document.querySelector("#hangar-close"); this.hangarSlots = document.querySelector("#hangar-slots"); this.hangarMessage = document.querySelector("#hangar-message");
    ...
    this.moduleGlowToggle.checked = false; setModuleGlowEnabled(false); this.localSave = new LocalSaveSystem(); this.activeHangarSlot = this.localSave.getSelectedHangarSlot(); this.hangarCodeSlot = null; this.loadout = this.localSave.getHangarLoadout(this.activeHangarSlot) ?? createLoadout(); this.history = []; this.dragState = null; this.suppressClick = false; this.instanceCounter = 1; this.selectedMode = "endless"; this.selectedLevel = 1; this.quickAssemblyEnabled = false; this.quickAssemblyModuleId = null; this.tutorialActionsDone = new Set(); this.tutorialContext = null; this.tutorialBuilderInitialSignature = "";
    ...
    bindTap(this.hangarButton, () => this.openHangar()); bindTap(this.hangarClose, () => this.hideHangar()); this.hangarModal?.addEventListener("click", (event) => { if (event.target === this.hangarModal) this.hideHangar(); }); this.hangarSlots?.addEventListener("click", (event) => { const rename = event.target.closest("[data-hangar-rename]"); const code = event.target.closest("[data-hangar-code]"); const load = event.target.closest("[data-hangar-load]"); const save = event.target.closest("[data-hangar-save]"); if (rename) { this.startRenameHangarSlot(Number(rename.dataset.hangarRename)); return; } if (code) { this.openHangarSlotCode(Number(code.dataset.hangarCode)); return; } if (load) this.loadHangarSlot(Number(load.dataset.hangarLoad)); if (save) this.saveHangarSlot(Number(save.dataset.hangarSave)); });
  ...
  saveBuild() { if (this.tutorialContext === "builder") { const installedModules = this.loadout.modules.map(({ module }) => module).filter(Boolean); const hasAutomaticWeapon = installedModules.some((module) => module.type === "weapon"); const hasActiveSkill = installedModules.some((module) => Boolean(module.skill)); if (!hasAutomaticWeapon || !hasActiveSkill) { this.setStatus(`教程要求同时装配自动模块和技能模块（当前${hasAutomaticWeapon ? "已" : "未"}装配自动模块，${hasActiveSkill ? "已" : "未"}装配技能模块）`); return; } this.history = []; this.tutorialActionsDone.add(0); this.tutorialContext = null; this.showTutorial(1, false); return; } this.persistLocalLoadout(); this.history = []; this.onMenu?.(); }
  persistLocalLoadout() { if (this.beyondBuilderMode || this.tutorialContext) return false; return this.localSave?.saveHangarLoadout(this.activeHangarSlot, this.loadout) ?? false; }
  setHangarVisible(visible) { this.hangarButton?.classList.toggle("is-hidden", !visible); if (!visible) this.hideHangar(); }
  openHangar() { if (this.beyondBuilderMode || this.tutorialContext) return; this.renderHangar(); this.hangarModal?.classList.remove("is-hidden"); }
  hideHangar() { this.hangarModal?.classList.add("is-hidden"); }
  renderHangar(message = "") {
    if (!this.hangarSlots) return;
    const slots = this.localSave?.getHangarSlots() ?? [];
    this.hangarSlots.innerHTML = Array.from({ length: HANGAR_SLOT_COUNT }, (_, index) => {
      const loadout = slots[index]; const active = index === this.activeHangarSlot; const count = loadout?.modules?.length ?? 0;
      const name = loadout ? escapeHtml(this.localSave?.getHangarName(index) ?? `机体 ${index + 1}`) : "";
      const title = loadout ? `<button class="hangar-slot-name" type="button" data-hangar-rename="${index}" aria-label="重命名机体 ${index + 1}">${name}</button>` : "<strong>空槽位</strong>";
      return `<article class="hangar-slot ${active ? "is-active" : ""} ${loadout ? "is-occupied" : "is-empty"}" data-hangar-slot="${index}"><div><span class="hangar-slot-number">SLOT ${String(index + 1).padStart(2, "0")}</span>${title}<small>${loadout ? `${count} 个模块` : "尚未保存"}</small></div><div class="hangar-slot-actions"><button class="secondary-button" type="button" data-hangar-code="${index}">配置码</button><button class="primary-button" type="button" data-hangar-save="${index}">存储机体</button><button class="secondary-button" type="button" data-hangar-load="${index}" ${loadout ? "" : "disabled"}>应用机体</button></div></article>`;
    }).join("");
    if (this.hangarMessage) this.hangarMessage.textContent = message;
  }
  startRenameHangarSlot(slot) {
    const loadout = this.localSave?.getHangarLoadout(slot); const display = this.hangarSlots?.querySelector(`[data-hangar-rename="${slot}"]`);
    if (!loadout || !display) return;
    const input = document.createElement("input"); input.className = "hangar-slot-name-input"; input.type = "text"; input.value = this.localSave.getHangarName(slot); input.maxLength = 24; input.autocomplete = "off"; input.inputMode = "text"; input.enterKeyHint = "done"; input.setAttribute("aria-label", `编辑机体名称 ${slot + 1}`);
    let finished = false;
    const commit = () => { if (finished) return; finished = true; this.localSave.renameHangarSlot(slot, input.value); this.renderHangar(); this.setStatus(`机体名称已更新为“${this.localSave.getHangarName(slot)}”`); };
    const cancel = () => { if (finished) return; finished = true; this.renderHangar(); };
    ...
    input.addEventListener("blur", commit, { once: true }); display.replaceWith(input);
    input.focus({ preventScroll: true }); input.select();
  }
  openHangarSlotCode(slot) {
    if (!Number.isInteger(slot) || !this.loadoutCodeModal || !this.loadoutCodeInput) return;
    const loadout = this.localSave?.getHangarLoadout(slot);
    this.hangarCodeSlot = slot;
    if (loadout) {
      try { this.loadoutCodeInput.value = encodeLoadoutCode(loadout); this.loadoutCodeMessage.textContent = `槽位 ${slot + 1} 的配置码已生成，可复制或修改后导入`; } catch (error) { this.hangarCodeSlot = null; this.setStatus(error instanceof Error ? error.message : "无法生成配置码"); return; }
    } else {
      this.loadoutCodeInput.value = ""; this.loadoutCodeMessage.textContent = `请输入配置码，将导入到机库槽位 ${slot + 1}`;
    }
    this.hideHangar(); this.loadoutCodeModal.classList.remove("is-hidden"); requestAnimationFrame(() => { this.loadoutCodeInput.focus(); this.loadoutCodeInput.select(); });
  }
  loadHangarSlot(slot) { const loadout = this.localSave?.getHangarLoadout(slot); if (!loadout) { this.renderHangar("这个槽位还没有机体"); return; } this.pushHistory(); this.activeHangarSlot = slot; this.localSave.selectHangarSlot(slot); this.loadout = loadout; this.history = []; this.renderAssembly(); this.hideHangar(); this.setStatus(`已应用机体 ${slot + 1}`); }
  saveHangarSlot(slot) { const result = validateLoadout(this.loadout); if (!result.valid) { this.renderHangar(result.reason); return; } this.activeHangarSlot = slot; this.localSave.saveHangarLoadout(slot, this.loadout); this.renderHangar(`已存储到机库槽位 ${slot + 1}`); this.setStatus(`当前机体已存储到槽位 ${slot + 1}`); }
  ...
  hideLoadoutCode() { this.loadoutCodeModal?.classList.add("is-hidden"); this.hangarCodeSlot = null; }
  ...
  async copyLoadoutCode() { try { const code = this.hangarCodeSlot === null ? encodeLoadoutCode(this.loadout) : this.loadoutCodeInput.value.trim(); if (!code) throw new Error("没有可复制的配置码"); this.loadoutCodeInput.value = code; let copied = false; if (navigator.clipboard?.writeText) { await navigator.clipboard.writeText(code); copied = true; } if (!copied) { this.loadoutCodeInput.focus(); this.loadoutCodeInput.select(); try { copied = document.execCommand("copy"); } catch {} } this.loadoutCodeMessage.textContent = copied ? "配置码已复制" : "配置码已生成，可长按或全选复制"; } catch (error) { this.loadoutCodeMessage.textContent = error instanceof Error ? error.message : "无法复制配置码"; } }
  importLoadoutCode() { try { const { loadout, moduleCount } = decodeLoadoutCode(this.loadoutCodeInput.value); if (this.hangarCodeSlot !== null) { const slot = this.hangarCodeSlot; this.pushHistory(); this.activeHangarSlot = slot; this.loadout = loadout; this.instanceCounter = Math.max(this.instanceCounter, moduleCount + 1); this.localSave.saveHangarLoadout(slot, loadout); this.renderAssembly(); this.hideLoadoutCode(); this.openHangar(); this.setStatus(`配置码已导入机库槽位 ${slot + 1}`); return; } this.pushHistory(); this.loadout = loadout; this.instanceCounter = Math.max(this.instanceCounter, moduleCount + 1); this.renderAssembly(); this.hideLoadoutCode(); this.setStatus(`已导入 ${moduleCount} 个模块`); } catch (error) { this.loadoutCodeMessage.textContent = error instanceof Error ? error.message : "配置码无效"; } }
  ...
  transitionToScreen(target, { direction = "forward", animate = true } = {}) {
    const screens = [this.menuScreen, this.modeScreen, this.dodgeDifficultyScreen, this.tutorialScreen, this.levelSelectScreen, this.encyclopediaScreen, this.encyclopediaEventsScreen, this.encyclopediaUnreleasedEnemiesScreen, this.builderScreen, this.battleScreen].filter(Boolean);
  ...
  hideAllScreens() {
    ...
    if (this.screenTransitionTimer) window.clearTimeout(this.screenTransitionTimer);
    this.screenTransitionTimer = null;
    [this.menuScreen, this.modeScreen, this.dodgeDifficultyScreen, this.tutorialScreen, this.levelSelectScreen, this.encyclopediaScreen, this.encyclopediaEventsScreen, this.encyclopediaUnreleasedEnemiesScreen, this.builderScreen, this.battleScreen].forEach((screen) => {
  ...
  showMenu({ direction = "back" } = {}) { if (this.beyondReturnLoadout) { this.loadout = this.beyondReturnLoadout; this.beyondReturnLoadout = null; this.beyondBuilderMode = true; this.renderModuleLibrary(); this.renderAssembly(); this.beyondBuilderMode = false; } this.setHangarVisible(true); this.transitionToScreen(this.menuScreen, { direction }); this.menuAmbient?.start(); this.gameOverScreen.classList.add("is-hidden"); this.hidePause(); this.hideModuleDetail(); this.hideLoadoutCode(); this.hideHangar(); this.hideAttackSpeedRules(); this.hideChangelog(); this.settingsModal.classList.add("is-hidden"); this.hideMoreMenu(); }
  showModeSelect({ direction = "forward" } = {}) { const subtitle = this.levelModeButton?.querySelector("small"); if (subtitle) subtitle.textContent = "MISSION / 100 个战区"; this.transitionToScreen(this.modeScreen, { direction }); }
  ...
  showUnreleasedEnemyArchive({ direction = "forward" } = {}) { this.renderUnreleasedEnemyArchive(); this.transitionToScreen(this.encyclopediaUnreleasedEnemiesScreen, { direction }); }
  ...
  renderEncyclopedia() {
    this.encyclopediaUnreleasedEnemiesScreen?.remove();
    ...
    this.encyclopediaContent.querySelectorAll("[data-encyclopedia-action]").forEach((button) => bindTap(button, () => { if (button.dataset.encyclopediaAction === "beyond-events") this.showBeyondEventArchive(); if (button.dataset.encyclopediaAction === "unreleased-enemies") this.showUnreleasedEnemyArchive(); }));
  ...
  renderUnreleasedEnemyArchive() {
    if (!this.encyclopediaUnreleasedEnemiesContent) return;
    const entries = getUnreleasedEnemyArchiveEntries();
    this.encyclopediaUnreleasedEnemiesContent.innerHTML = `<section class="encyclopedia-section"><h3>原型目录 / ${entries.length}</h3><div class="encyclopedia-entries">${entries.map((entry, index) => `<article class="encyclopedia-entry has-enemy-visual"><div class="encyclopedia-copy"><div><strong>${escapeHtml(entry.title)}</strong><small>${escapeHtml(entry.tag)}</small></div><p>${escapeHtml(entry.description)}</p><span>${escapeHtml(entry.meta)}</span></div><canvas class="encyclopedia-enemy-canvas" width="64" height="64" data-unreleased-enemy-entry="${index}"></canvas></article>`).join("")}</div></section>`;
    entries.forEach((entry, index) => paintEncyclopediaEnemy(this.encyclopediaUnreleasedEnemiesContent.querySelector(`[data-unreleased-enemy-entry="${index}"]`), entry.enemyVisual));
  }
  ...
  renderAssembly() {
    ...
    const world = `<div class="assembly-board-world"><div class="board-grid-lines" aria-hidden="true"></div><div class="board-cells" aria-hidden="true">${cells}</div><div class="board-core" style="left:${(core.x / cols) * 100}%;top:${(core.y / rows) * 100}%;width:${100 / cols}%;height:${100 / rows}%"><canvas class="board-icon-canvas" width="28" height="28" data-icon-module="core-stellar" aria-hidden="true"></canvas></div>${placed}</div>`;
     this.assemblyBoard.innerHTML = world; this.quickPreview = null; this.paintIcons(this.assemblyBoard); this.applyBoardView(); this.updatePreview(); this.persistLocalLoadout(); if (!this.synergyConfigModal?.classList.contains("is-hidden")) this.renderSynergyConfig();
  }
