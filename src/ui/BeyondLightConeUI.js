...
export function installBeyondLightConeUI(ui) {
  ...
  map.addEventListener("pointerup", (event) => { const button = event.target.closest?.("[data-node]"); const node = ui.beyondRun?.map?.flat().find((item) => item.id === button?.dataset.node); if (!node || node.type !== "boss") return; requestAnimationFrame(() => { const reward = getModuleById(node.bossRewardId); $("#beyond-node-title").textContent = `✹ ${node.bossName ?? "首领"}`; $("#beyond-node-description").textContent = "这位首领守护着一件传说模块。击败它即可获得奖励并恢复满血。"; $("#beyond-node-rewards").textContent = reward?.rarity === "legendary" ? `固定奖励：${reward.name}` : "奖励：随机开放传说模块"; }); }, true);
